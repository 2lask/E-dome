...
export default function OnboardingPage() {
  ...
  const inputClass =
    "w-full px-4 py-3 rounded-xl bg-[var(--input-bg)] border border-[var(--input-border)] text-[var(--foreground)] placeholder:text-[var(--text-muted)] focus:outline-none focus:border-[var(--primary)] focus:ring-1 focus:ring-[var(--primary)]/30 transition-colors";
            ...
            {Array.from({ length: totalSteps }, (_, i) => (
              <div
                ...
                className={`h-1.5 flex-1 rounded-full transition-colors duration-300 ${
                  i < step ? "bg-[var(--primary)]" : "bg-[var(--input-bg)]"
                }`}
              />
            ))}
          </div>
            ...
            <div className="animate-fade-in">
              ...
              <div className="mb-8 p-4 rounded-xl bg-[var(--background)] border border-[var(--card-border)]">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm text-[var(--primary)] font-semibold">{profileCompletion()}%</span>
                </div>
              ...
              <div className="flex items-center gap-4 mb-6">
                <div className="w-20 h-20 rounded-2xl bg-[var(--input-bg)] border-2 border-dashed border-[var(--input-border)] flex items-center justify-center cursor-pointer hover:border-[var(--primary)] transition-colors group">
                ...
                <div>
                  ...
                  <button className="text-xs text-[var(--primary)] hover:underline mt-1">
            ...
            <div className="animate-fade-in">
                  ...
                  >
                    ...
                    <div
                      className={`absolute top-3 right-3 w-8 h-5 rounded-full transition-colors flex items-center ${
                        selectedRoles.includes(id) ? "bg-[var(--primary)]" : "bg-[var(--input-bg)]"
                      }`}
                    >
                      <div
                    ...
                    <Icon
                      className={`w-6 h-6 mb-2 ${
                        selectedRoles.includes(id) ? "text-[var(--primary)]" : "text-[var(--text-muted)]"
                      }`}
                    />
              ...
              {selectedRoles.length > 0 && (
                <p className="mt-4 text-sm text-[var(--text-secondary)]">
                  <span className="text-[var(--primary)] font-semibold">{selectedRoles.length}</span>{" "}
                ...
                {getSuggestions().map((suggestion, i) => (
                  ...
                  >
                    <div className="w-10 h-10 rounded-xl bg-[var(--primary)]/10 flex items-center justify-center flex-shrink-0 group-hover:bg-[var(--primary)]/20 transition-colors">
                      <suggestion.icon className="w-5 h-5 text-[var(--primary)]" />
                    </div>
        ...
        <div className="sticky bottom-[calc(80px+env(safe-area-inset-bottom))] md:bottom-0 bg-[var(--card)] border-t border-[var(--divider)] p-6 rounded-b-2xl">
          <div className="flex gap-3">
            ...
            {step < totalSteps ? (
              <button
                onClick={() => setStep(step + 1)}
                className="flex-1 py-3 rounded-xl bg-[var(--primary)] hover:bg-[var(--primary)] text-white font-semibold flex items-center justify-center gap-2 transition-colors"
              >
                Suivant
