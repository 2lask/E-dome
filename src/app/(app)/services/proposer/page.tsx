    ...
    return (
      <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)] flex items-center justify-center p-4">
        <div className="text-center animate-scale-in max-w-md">
          ...
          <h1 className="text-2xl page-heading mb-2">Service publié !</h1>
          <p className="text-[var(--text-secondary)] mb-6">Votre service &quot;{form.titre}&quot; est maintenant disponible sur la plateforme.</p>
          <a href="/services" className="px-6 py-3 bg-[var(--primary)] hover:bg-[var(--primary)] text-white rounded-xl font-medium transition-colors inline-block">
            Voir les services
          </a>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
      <div className="max-w-2xl mx-auto px-4 py-8">
        <h1 className="text-3xl page-heading mb-2">Proposer un service</h1>
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {CATEGORIES.map((c) => (
                <button key={c} onClick={() => update("categorie", c)} className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${form.categorie === c ? "bg-[var(--primary)] text-white" : "bg-[var(--card)] border border-[var(--card-border)] text-[var(--text-secondary)] hover:border-[var(--primary)]/40"}`}>
                  {c}
                </button>
              ))}
            </div>
            ...
            <div className="flex gap-2">
              {JOURS.map((jour, i) => (
                <button
                  key={jour}
                  onClick={() => toggleDay(i)}
                  className={`flex-1 py-3 rounded-xl text-sm font-medium transition-colors ${form.disponibilites[i] ? "bg-[var(--primary)] text-white" : "bg-[var(--card)] border border-[var(--card-border)] text-[var(--text-muted)]"}`}
                >
                  {jour}
                </button>
              ))}
          ...
          <div>
            ...
            <div className="flex gap-2 mb-2">
              ...
              <button onClick={addPhoto} className="px-4 py-3 bg-[var(--primary)] hover:bg-[var(--primary)] text-white rounded-xl font-medium transition-colors">
                Ajouter
              </button>
            </div>
          ...
          <button onClick={handlePublish} className="w-full py-3 bg-[var(--primary)] hover:bg-[var(--primary)] text-white rounded-xl font-medium transition-colors mt-4">
            Publier le service
          </button>
        </div>
      </div>
