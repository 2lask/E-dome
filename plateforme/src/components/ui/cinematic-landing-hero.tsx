...
export interface CinematicHeroProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  /** Optional override for the two CTA buttons block. Defaults to App Store / Google Play. */
  ctaButtons?: React.ReactNode;
  /** Optional override for the two floating glass badges around the iPhone mockup. */
  floatingBadgeTop?: { icon?: React.ReactNode; title: string; subtitle: string };
  floatingBadgeBottom?: { icon?: React.ReactNode; title: string; subtitle: string };
  /** Optional today / journey microcopy at the top of the iPhone screen. */
  phoneEyebrow?: string;
  phoneTitle?: string;
  phoneAvatar?: string;
}
...
export function CinematicHero({
  ...
  ctaButtons,
  floatingBadgeTop = { icon: "🔥", title: "1 Year Streak", subtitle: "Milestone unlocked" },
  floatingBadgeBottom = { icon: "🤝", title: "Sponsor Update", subtitle: "Shared successfully" },
  phoneEyebrow = "Today",
  phoneTitle = "Journey",
  phoneAvatar = "JS",
  className,
              ...
              <div className="relative w-full h-full flex items-center justify-center transform scale-[0.65] md:scale-85 lg:scale-100">
                ...
                <div className="floating-badge absolute flex top-6 lg:top-12 left-[-15px] lg:left-[-80px] floating-ui-badge rounded-xl lg:rounded-2xl p-3 lg:p-4 items-center gap-3 lg:gap-4 z-30">
                  <div className="w-8 h-8 lg:w-10 lg:h-10 rounded-full bg-gradient-to-b from-blue-500/20 to-blue-900/10 flex items-center justify-center border border-blue-400/30 shadow-inner">
                    <span className="text-base lg:text-xl drop-shadow-lg" aria-hidden="true">{floatingBadgeTop.icon ?? "🔥"}</span>
                  </div>
                  <div>
                    <p className="text-white text-xs lg:text-sm font-bold tracking-tight">{floatingBadgeTop.title}</p>
                    <p className="text-blue-200/50 text-[10px] lg:text-xs font-medium">{floatingBadgeTop.subtitle}</p>
                  </div>
                </div>
                ...
                <div className="floating-badge absolute flex bottom-12 lg:bottom-20 right-[-15px] lg:right-[-80px] floating-ui-badge rounded-xl lg:rounded-2xl p-3 lg:p-4 items-center gap-3 lg:gap-4 z-30">
                  <div className="w-8 h-8 lg:w-10 lg:h-10 rounded-full bg-gradient-to-b from-indigo-500/20 to-indigo-900/10 flex items-center justify-center border border-indigo-400/30 shadow-inner">
                    <span className="text-base lg:text-lg drop-shadow-lg" aria-hidden="true">{floatingBadgeBottom.icon ?? "🤝"}</span>
                  </div>
                  <div>
                    <p className="text-white text-xs lg:text-sm font-bold tracking-tight">{floatingBadgeBottom.title}</p>
                    <p className="text-blue-200/50 text-[10px] lg:text-xs font-medium">{floatingBadgeBottom.subtitle}</p>
                  </div>
                </div>
              </div>
            </div>
