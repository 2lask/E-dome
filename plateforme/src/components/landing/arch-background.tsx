...
export function ArchBackground({ variant, className = "" }: ArchBackgroundProps) {
  ...
  return (
    <div ref={ref} className={`absolute inset-0 overflow-hidden pointer-events-none ${className}`}>
      {variant === "villa" && (
        <motion.div className="absolute right-[-15%] top-[0%] w-[95%] md:w-[75%] text-white/[0.09]" style={{ y: y1, opacity, x: x1 }}>
          <VillaLuxuryDrawing />
        </motion.div>
      )}
      ...
      {variant === "floorplan" && (
        <>
          <motion.div className="absolute left-[-10%] top-[-5%] w-[85%] md:w-[60%] text-white/[0.07]" style={{ y: y2, opacity, x: x2 }}>
            <ChaletAlpineDrawing />
          </motion.div>
          <motion.div className="absolute right-[-15%] bottom-[-5%] w-[70%] md:w-[50%] text-white/[0.05]" style={{ y: y1, opacity, x: x1 }}>
            <VillaLuxuryDrawing />
          </motion.div>
        </>
      )}
      ...
      {variant === "building" && (
        <motion.div className="absolute right-[-5%] top-[-10%] w-[55%] md:w-[40%] text-white/[0.08]" style={{ y: y1, opacity, x: x1 }}>
      ...
      {variant === "mixed" && (
        <>
          <motion.div className="absolute left-[-12%] top-[-5%] w-[80%] md:w-[55%] text-white/[0.06]" style={{ y: y2, opacity, x: x2 }}>
            <InteriorPenthouseDrawing />
          </motion.div>
          <motion.div className="absolute right-[-10%] bottom-[-5%] w-[55%] md:w-[40%] text-white/[0.07]" style={{ y: y1, opacity, x: x1 }}>
            <TowerResidenceDrawing />
          </motion.div>
        </>
      )}
