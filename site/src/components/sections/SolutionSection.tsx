...
function PropertyCardMockup() {
  return (
    ...
    >
      <p className="mb-4 text-center text-sm font-medium uppercase tracking-widest text-[#AAAAAA]">
      ...
      <div className="group relative overflow-hidden rounded-2xl border border-[#8B6F47]/12 bg-white shadow-2xl shadow-black/40 transition-all duration-500 hover:border-[#8B6F47]/25 hover:shadow-[0_0_60px_-15px_rgba(255,224,194,0.12)]">
        <div className="relative h-48 w-full overflow-hidden">
          ...
          <div className="absolute bottom-4 right-4 rounded-full border border-white/10 bg-[#FAF8F5]/70 p-2 backdrop-blur-sm">
            <Store className="h-4 w-4 text-[#888888]" />
          </div>
        </div>
        <div className="p-5">
          <div className="flex items-start justify-between">
            <div>
              <h5 className="text-base font-bold text-[#1A1A1A]">Villa Méditerranée</h5>
              <div className="mt-1 flex items-center gap-1.5 text-[#888888]">
                <MapPin className="h-3.5 w-3.5" />
                <span className="text-xs">Marrakech, Maroc</span>
              </div>
            </div>
            <div className="text-right">
              <p className="text-lg font-bold bg-gradient-to-r from-[#8B6F47] to-[#C4956A] bg-clip-text text-transparent">245 CHF</p>
              <p className="text-[10px] text-[#AAAAAA] font-medium">/ nuit</p>
            </div>
          </div>
          <div className="mt-3 flex items-center gap-1">
            {[1, 2, 3, 4, 5].map((s) => (
              <Star key={s} className={cn("h-3.5 w-3.5", s <= 4 ? "fill-[#8B6F47] text-[#8B6F47]" : "fill-none text-[#CCCCCC]")} />
            ))}
            <span className="ml-1.5 text-xs text-[#888888]">4.0 (128)</span>
          </div>
          <div className="my-4 h-px bg-gradient-to-r from-transparent via-[#201e18] to-transparent" />
          <button className="w-full rounded-xl bg-gradient-to-r from-[#8B6F47] to-[#C4956A] px-4 py-2.5 text-sm font-semibold text-[#1A1A1A] transition-all duration-300 hover:opacity-90 hover:shadow-[0_0_24px_-6px_rgba(255,224,194,0.35)]">
            Réserver
          </button>
        </div>
      </div>
...
export default function SolutionSection() {
  return (
    <section id="solution" className="relative bg-[#FAF8F5]/90 py-32 px-6 md:px-12 arch-bg-grid arch-bg-radial">
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute top-1/3 left-1/4 h-[500px] w-[500px] rounded-full bg-[#8B6F47]/5 blur-[140px]" />
        <div className="absolute bottom-1/4 right-1/4 h-[400px] w-[400px] rounded-full bg-[#C4956A]/5 blur-[120px]" />
      </div>
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
          ...
          >
            ...
            >
              {featurePills.map((pill) => {
                ...
                return (
                  <span key={pill.label} className="inline-flex items-center gap-2 rounded-full border border-[#8B6F47]/12 bg-white/50 px-4 py-2 text-sm text-[#333333] transition-colors hover:border-[#8B6F47]/30">
        ...
        <div className="mt-32">
          ...
          >
            <span className="inline-flex items-center rounded-full border border-[#8B6F47]/20 bg-[#8B6F47]/5 px-4 py-1.5 text-sm font-medium text-[#8B6F47]">
              <span className="mr-2 inline-block h-1.5 w-1.5 rounded-full bg-[#C4956A] animate-pulse" />
              Les 5 Piliers
            </span>
            <h3 className="mt-6 text-3xl font-bold tracking-tight text-[#1A1A1A] md:text-5xl">
              Cinq piliers.{" "}
              <span className="bg-gradient-to-r from-[#8B6F47] to-[#C4956A] bg-clip-text text-transparent">
                Un seul écosystème.
              </span>
            ...
            {pillars.map((pillar, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className={cn(
                    "group relative flex min-h-[310px] flex-col rounded-2xl border border-[#8B6F47]/12 bg-white p-6 md:p-8",
                    "transition-all duration-500",
                    "hover:border-[#8B6F47]/30 hover:shadow-[0_0_50px_-10px_rgba(255,224,194,0.13)]",
                    idx === 4 && "sm:col-span-2 lg:col-span-1"
                  )}
                >
                  {/* Card number */}
                  <span className="absolute top-4 right-5 text-5xl font-bold text-[#1A1A1A]/[0.03] select-none">
                    {String(idx + 1).padStart(2, "0")}
                  </span>
                  <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[#8B6F47]/15 to-transparent rounded-t-2xl" />
                  <div className="relative mb-6 inline-flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br from-[#8B6F47]/15 to-[#C4956A]/10 ring-1 ring-[#8B6F47]/15 transition-all group-hover:ring-[#8B6F47]/30 group-hover:shadow-[0_0_20px_-4px_rgba(255,224,194,0.25)]">
                    <Icon className="h-6 w-6 text-[#8B6F47] transition-transform duration-300 group-hover:scale-110" />
                  </div>
                  <h4 className="mb-3 text-lg font-bold text-[#1A1A1A]">{pillar.title}</h4>
                  <p className="mb-auto text-sm leading-relaxed text-[#888888]">{pillar.description}</p>
                  <div className="mt-5 inline-flex items-center gap-2 self-start rounded-full border border-[#8B6F47]/10 bg-[#8B6F47]/[0.06] px-3.5 py-1.5 transition-colors duration-300 group-hover:border-[#8B6F47]/20 group-hover:bg-[#8B6F47]/[0.1]">
                    <span className="inline-block h-1.5 w-1.5 shrink-0 rounded-full bg-[#8B6F47]" />
                    <p className="text-xs font-medium text-[#8B6F47]/80">{pillar.impact}</p>
                  </div>
                  <div className="absolute bottom-0 left-0 h-[2px] w-0 rounded-b-2xl bg-gradient-to-r from-[#8B6F47] to-[#C4956A] transition-all duration-700 group-hover:w-full" />
                </motion.div>
              );
            })}
          </motion.div>
